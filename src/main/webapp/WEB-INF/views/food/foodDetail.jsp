...
<c:forEach var="dto" items="${list }" varStatus="status">
	<h1>${dto.name } 상세페이지</h1>
	<hr>
	<form action="cartInsert.do" method="post">
	<img src="/artHouse/resources/img/food/${dto.img }">
	<p class="food_name">${dto.name }</p>
	<p class="food_price">${dto.price }</p>
	<input type="hidden" name="code" value="${dto.code }">  <!-- 상품코드 -->
	<input type="hidden" name="price" value="${dto.price }">  <!-- 상품가격 -->
	<input type="hidden" name="name" value="${dto.name }">  <!-- 상품명 -->
